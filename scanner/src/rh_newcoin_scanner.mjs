...
const CFG = {
  ...
  ponsV1Factory: norm(process.env.PONS_V1_FACTORY || '0xa5aab3f0c6eeadf30ef1d3eb997108e976351feb'),
...
const client = createPublicClient({
  ...
  transport: viemHttp(CFG.rpcUrl, { timeout: 15000, retryCount: 2 }),
});
...
function topicAddress(topic) {
  ...
  return norm('0x' + topic.slice(26));
}

function isQuote(addr) {
  return addr && QUOTES.has(addr.toLowerCase());
}
...
function deDupeKey(payload) {
  return [payload.stage, payload.tokenCa, payload.pool, payload.txHash].filter(Boolean).join('|').toLowerCase();
}
...
async function postWebhook(payload) {
  ...
  try {
    ...
    console.error('[webhook]', state.lastError);
...
async function scanPonsV2(fromBlock, toBlock) {
  ...
  const logs = await client.getLogs({
    address: getAddress(CFG.ponsV2Factory),
    fromBlock,
    toBlock,
  });
  ...
  for (const log of logs) {
    ...
    if (topic0 === PONS_TOKEN_LAUNCHED_TOPIC) {
      ...
      await emit({
        ...
        deployer,
        pairToken,
        pairType: isQuote(pairToken) ? 'Quote' : '',
...
async function scanV4(fromBlock, toBlock) {
  ...
  const logs = await client.getLogs({
    address: getAddress(CFG.v4PoolManager),
    event: V4_INIT,
    fromBlock,
    toBlock,
  });
  ...
  for (const log of logs) {
    try {
      ...
      const a = norm(decoded.args.currency0);
      const b = norm(decoded.args.currency1);
      ...
      await emit({
        ...baseLogPayload(log),
        source: 'Uniswap V4',
        stage: 'V4 PoolInitialized',
        tokenCa: token,
        pool: String(decoded.args.id || ''),
        pairToken: quote,
        pairType: 'V4',
      ...
      console.warn('[v4 decode]', String(err?.message || err));
    }
  }
}
...
async function scanV3(fromBlock, toBlock) {
  ...
  const logs = await client.getLogs({
    address: getAddress(CFG.v3Factory),
  ...
  for (const log of logs) {
    try {
      ...
      const a = norm(decoded.args.token0);
      const b = norm(decoded.args.token1);
      const token = tokenFromPair(a, b);
      if (!token) continue;
      const quote = isQuote(a) ? a : b;
      await emit({
        ...baseLogPayload(log),
        source: 'Uniswap V3',
        stage: 'V3 PoolCreated',
        tokenCa: token,
        pool: norm(decoded.args.pool),
        pairToken: quote,
        pairType: 'V3',
        notes: `fee=${decoded.args.fee}; tickSpacing=${decoded.args.tickSpacing}`,
        enrichment: 'raw-chain',
      });
    } catch (err) {
      console.warn('[v3 decode]', String(err?.message || err));
    }
  }
}
...
async function heartbeat() {
  ...
  await postWebhook({
    kind: 'heartbeat',
    heartbeat: state.lastHeartbeatAt,
    latestBlock: state.lastBlock,
    provider: 'Robinhood RPC polling',
    listeners: {
      'Pons V2': Boolean(CFG.ponsV2Factory),
      'Uniswap V4': Boolean(CFG.v4PoolManager),
      'Uniswap V3': Boolean(CFG.v3Factory),
    },
  });
}
...
async function mainLoop() {
  ...
  console.log('[boot]', JSON.stringify({
    ...
    webhookConfigured: Boolean(CFG.webhookUrl && CFG.webhookSecret),
    dryRun: CFG.dryRun,
    ponsV2Factory: CFG.ponsV2Factory,
    v4PoolManager: CFG.v4PoolManager,
    v3Factory: CFG.v3Factory || null,
    quoteTokens: [...QUOTES],
  }));
...
setInterval(() => heartbeat().catch((err) => console.error('[heartbeat]', err)), CFG.heartbeatMs).unref();
heartbeat().catch((err) => console.error('[heartbeat]', err));
mainLoop().catch((err) => {
